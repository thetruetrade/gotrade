...
func NewPlusDIWithoutStorage(timePeriod int, valueAvailableAction ValueAvailableAction) (indicator *PlusDIWithoutStorage, err error) {
	var lookback int = 1
	if timePeriod > 1 {
		lookback = timePeriod
	}
	newPlusDI := PlusDIWithoutStorage{baseIndicator: newBaseIndicator(lookback),
